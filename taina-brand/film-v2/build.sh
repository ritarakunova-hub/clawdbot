#!/usr/bin/env bash
# Сборка ролика: кадры (Playwright + Chromium) + звук (numpy), вертикаль 1080×1920 → taina-film-vertical.mp4
...
"$FF" -y -loglevel error -framerate 30 -i "$TMP/frames/%04d.jpg" -i "$TMP/sound.wav" \
  ...
  -c:a aac -b:a 192k -shortest taina-film-vertical.mp4
rm -rf "$TMP"
echo "Готово: $(pwd)/taina-film-vertical.mp4"
